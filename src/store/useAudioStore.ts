...
export const useAudioStore = create<AudioStore>((set, get) => ({
  ...
  playMusic: async (type: AudioType) => {
    ...
    if (!manager) {
      const newManager = new AudioManager(type)
      newManager.setVolume(musicVolume)

      newManager.onPlay(() => {
        set({ isMusicPlaying: true, currentMusicType: type })
      })

      newManager.onPause(() => set({ isMusicPlaying: false }))
      newManager.onEnded(() => set({ isMusicPlaying: false }))


      newManager.onTrackChange(() => {
        const info = newManager.getCurrentTrackInfo()
        set({ currentTrackInfo: info ? { title: info.title, artist: info.artist } : null })
      })

      musicManagers.set(type, newManager)
      set({ musicManagers })
    ...
    if (manager) {
      try {
        ...
        const info = manager.getCurrentTrackInfo()
        set({ currentTrackInfo: info ? { title: info.title, artist: info.artist } : null })
